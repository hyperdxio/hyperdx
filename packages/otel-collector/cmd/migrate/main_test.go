...
func TestClassifySignal(t *testing.T) {
	cases := map[string]string{
		...
		"otel_logs_key_rollup_15m":           "LOGS_TTL", // seeded by an earlier schema
		"otel_traces":                        "TRACES_TTL",
		"otel_traces_kv_rollup_15m":          "TRACES_TTL",
		"otel_traces_key_rollup_15m":         "TRACES_TTL",
	...
	// metrics_ts is the PromQL TimeSeries table (no TTL); the rest are not tables
	// this tool manages and must never have their TTL rewritten.
	for _, name := range []string{"metrics_ts", "some_other_table", "otel_logs_backup_2026", "otel_logs_archive", "otel_traces_copy"} {
...
func TestIntervalSeconds(t *testing.T) {
	cases := map[string]int64{
		...
		"toIntervalWeek(2)":    2 * 604800,
		"INTERVAL 3 WEEK":      3 * 604800,
	}
	...
	// Calendar units have no fixed length, so they must not convert to seconds.
	for _, term := range []string{"toIntervalMonth(6)", "toIntervalQuarter(2)", "toIntervalYear(1)", "INTERVAL 6 MONTH"} {
		if _, ok := intervalSeconds(term); ok {
			t.Errorf("intervalSeconds(%q) should return ok=false for a calendar unit", term)
		}
	}
}

func TestHasTopLevelComma(t *testing.T) {
	if !hasTopLevelComma("ts + toIntervalDay(7) TO VOLUME 'cold', ts + toIntervalDay(30)") {
		t.Error("expected true for a comma separating TTL rules")
	}
	if hasTopLevelComma("toDateTime(Timestamp, 'UTC') + toIntervalDay(30)") {
		t.Error("expected false for a comma nested inside a function call")
	}
}
...
func TestPlanTTLReconcile(t *testing.T) {
	...
	cases := []struct {
		...
		{"tiered TTL with mixed units refused", "otel_logs", ct("otel_logs", "toDateTime(Timestamp) + INTERVAL 30 DAY TO VOLUME 'cold', toDateTime(Timestamp) + INTERVAL 6 MONTH DELETE"), ttlWarn, "", false},
		{"tiered TTL with calendar move rule refused", "otel_logs", ct("otel_logs", "toDateTime(Timestamp) + toIntervalMonth(1) TO DISK 'slow', toDateTime(Timestamp) + toIntervalDay(30) DELETE"), ttlWarn, "", false},
		{"calendar-unit retention refused", "otel_logs", ct("otel_logs", "toDateTime(Timestamp) + toIntervalMonth(6)"), ttlWarn, "", false},
		{"GROUP BY rollup TTL refused", "otel_logs", ct("otel_logs", "toDateTime(Timestamp) + toIntervalDay(30) GROUP BY ServiceName SET Body = any(Body)"), ttlWarn, "", false},
		{"week unit is fixed-length and rewritable", "otel_logs", ct("otel_logs", "toDateTime(Timestamp) + toIntervalWeek(2)"), ttlAlter, "toDateTime(Timestamp) + toIntervalDay(180)", true},
	}
	for _, c := range cases {
