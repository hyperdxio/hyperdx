...
//
// Downstream distributions register extensions from
...
/**
 * @public Part of the extension seam contract consumed by downstream
 * distributions; it has no in-repo importer by design (OSS ships no
 * extensions), so it is marked `@public` for knip.
 */
...
/**
 * @public Part of the extension seam contract consumed by downstream
 * distributions; no in-repo importer by design.
 */
export interface AgentSessionStartResult {
  ...
  // the extension can correlate the run later. A later extension's duplicate
  // key wins silently, so keep keys extension-specific (e.g. `notebookId`).
