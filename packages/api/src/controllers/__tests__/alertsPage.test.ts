...
describe('buildAlertsPageFilter', () => {
  ...
  it('leaves $or to the cursor when saved_search widens the source filter', () => {
    // `source` spells "documents with no source field" as `$in: [..., null]`
    // rather than a second `$or`, which would overwrite the cursor's and
