...
`);
  ...
  it('fetches multi-series metric charts correctly', async () => {
    const now = new Date('2022-01-05').getTime();
    const runId = Math.random().toString(); // dedup watch mode runs
    const teamId = `test`;
    ...
    await clickhouse.bulkInsertTeamMetricStream(
      buildMetricSeries({
        ...
        points: [
          ...
          { value: 8, timestamp: now + ms('4m') },
          { value: 8, timestamp: now + ms('6m') },
          { value: 9, timestamp: now + ms('9m') },
          { value: 15, timestamp: now + ms('11m') },
          { value: 17, timestamp: now + ms('14m') },
          { value: 32, timestamp: now + ms('16m') },
          { value: 42, timestamp: now + ms('19m') },
        ],
      }),
    );

    // Rate: 0, 12, 638, 8670
    // Sum w/ series 1: 0, 19, 645, 8687
    await clickhouse.bulkInsertTeamMetricStream(
      buildMetricSeries({
        name: 'test.users',
        ...
        unit: 'Users',
        points: [
          { value: 3, timestamp: now - ms('1m') },
          { value: 3, timestamp: now },
          { value: 14, timestamp: now + ms('4m') },
          { value: 15, timestamp: now + ms('6m') },
          { value: 92, timestamp: now + ms('9m') },
          { value: 653, timestamp: now + ms('11m') },
          { value: 5897, timestamp: now + ms('14m') },
          { value: 9323, timestamp: now + ms('16m') },
          { value: 84626, timestamp: now + ms('19m') },
        ],
      }),
    );
...
Array [
  ...
  Object {
    "group": "test2",
    "series_0.data": 0.3333333333333333,
    "ts_bucket": 1641341100,
  },
  Object {
    "group": "test1",
    "series_0.data": 11.428571428571429,
    "ts_bucket": 1641341100,
  },
  Object {
    "group": "",
