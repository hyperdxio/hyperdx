...
function getFirstConnectionId(connections: { _id: unknown }[]): string {
  return connections[0] ? String(connections[0]._id) : '<CONNECTION_ID>';
}

// ─── Prompt content builders ─────────────────────────────────────────────────

function buildCreateDashboardPrompt(
...
function buildDashboardExamplesPrompt(
  traceSourceId: string,
  logSourceId: string,
  connectionId: string,
  pattern?: string,
...
function buildQueryGuidePrompt(): string {
