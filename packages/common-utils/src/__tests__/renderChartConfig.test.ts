...
describe('renderChartConfig', () => {
  ...
  describe('SELECT alias references in a Lucene WHERE', () => {
    ...
    // Combined-fix regression guard. Extracting the alias from a string-form
    // select that contains a ClickHouse parametric aggregate (the double-paren
    // `func(params)(args)` form) requires chSqlToAliasMap to parse that form
    // without throwing — the parser gap that forced the original revert of the
    // unknown-field fix. With both fixes in place, the alias is collected and a
    // Lucene WHERE referencing it resolves to a bare identifier rather than the
    // no-match predicate.
    it('resolves a Lucene WHERE reference to an alias on a parametric aggregate in a string select', async () => {
      const config: ChartConfigWithOptDateRange = {
        displayType: DisplayType.Table,
        connection: 'test-connection',
        from: {
        ...
        select: 'quantile(0.9)(value) AS p90, Body',
        where: 'p90:"5"',
        whereLanguage: 'lucene',
      };
