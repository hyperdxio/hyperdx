...
describe('renderChartConfig', () => {
  ...
  describe('mapContains functionality', () => {
    beforeEach(() => {
      // Mock materialized fields to enable fastifySQL processing
      mockMetadata.getMaterializedColumnsLookupTable.mockResolvedValue(
        new Map([["Attributes['key1']", 'materialized_key1']]),
      );
    });

    it('should add mapContains when single map access is detected', async () => {
      const config: ChartConfigWithOptDateRange = {
        displayType: DisplayType.Table,
        connection: 'test-connection',
        from: {
        ...
        select: [
          {
            aggFn: 'count',
            valueExpression: '',
            aggCondition: "Attributes['service'] = 'web'",
      ...
      expect(actual).toContain("mapContains(Attributes, 'service')");
    });

    it('should add multiple mapContains when multiple map accesses are detected', async () => {
      const config: ChartConfigWithOptDateRange = {
        displayType: DisplayType.Table,
        connection: 'test-connection',
        from: {
        ...
        select: [
          {
            ...
            valueExpression: '',
            aggCondition:
              "Attributes['service'] = 'web' AND ResourceAttributes['host'] = 'server1'",
            aggConditionLanguage: 'sql',
          },
          {
            aggFn: 'count',
            valueExpression: '',
            aggCondition: "OtherAttributes['foo'] = 'bar'",
            aggConditionLanguage: 'sql',
          },
        ],
        where: '',
      ...
      expect(actual).toContain(
        "mapContains(Attributes, 'service') AND mapContains(ResourceAttributes, 'host')",
      );
      expect(actual).toContain("mapContains(OtherAttributes, 'foo')");
    });

    it('should not add mapContains when no map access is detected', async () => {
      const config: ChartConfigWithOptDateRange = {
        displayType: DisplayType.Table,
        connection: 'test-connection',
        from: {
    ...
    it('should add mapContains in WHERE clause when map access is used', async () => {
      const config: ChartConfigWithOptDateRange = {
        displayType: DisplayType.Table,
        connection: 'test-connection',
        from: {
      ...
      expect(actual).toContain("mapContains(Attributes, 'user_id')");
    });

    it('should work with CTE queries that disable materialized fields lookup', async () => {
      // When using CTEs, materialized fields lookup should be skipped
      const config: ChartConfigWithOptDateRange = {
        displayType: DisplayType.Table,
        connection: 'test-connection',
        with: [
          {
            name: 'FilteredLogs',
            sql: chSql`SELECT * FROM logs WHERE level = 'error'`,
          },
        ],
        from: {
          databaseName: '',
          tableName: 'FilteredLogs',
        },
        select: [
          {
            aggFn: 'count',
            valueExpression: 'count(*)',
            aggCondition: "Attributes['service'] = 'api'",
            aggConditionLanguage: 'sql',
          },
        ],
        where: '',
      ...
      // Should still contain the map access (though mapContains may not be added due to no materialized fields with CTEs)
      expect(actual).toContain("Attributes['service']");
    });
  });
});
