...
| `metric-saturation` | "Shoppers are seeing slow/missing recommendations; frontend-proxy logs upstream 503s..." (blinded — culprit service not named) | `recommendation-service` JVM heap leak → GC-pause tail → latency shift → staggered pod-restart cadence. Root cause is **metrics-only** and requires real metric work: per-pod × per-pool heap gauges (3 out-of-phase pods, only `G1 Old Gen` leaks — un-grouped averages blur the sawtooth), a healthy JVM twin (`search-service`) with identical metric names, all five metric types. Distractors: deploy 15 min after leak onset, correlated-but-symptomatic thread-count gauge, flat CPU, healthy JVM twin, stable neighbor Summary. |
