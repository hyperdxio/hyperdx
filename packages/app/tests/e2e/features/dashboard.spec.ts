...
test.describe('Dashboard', { tag: ['@dashboard'] }, () => {
  ...
  test(
    'should add and remove alert on Number type chart',
    { tag: '@full-stack' },
    async () => {
      test.setTimeout(60000);
  ...
  test('should handle URL query params overriding saved query', async () => {
  ...
  test('should clear saved query when WHERE input is cleared and saved', async () => {
