...
test.describe('Chart Explorer Extended', { tag: ['@charts'] }, () => {
  ...
  test('should run a query and see chart types available', async () => {
    ...
    await test.step('Verify all chart type tabs are available', async () => {
      await expect(
        chartExplorerPage.page.getByTestId('chart-type-line'),
      ).toBeVisible();
      await expect(
        chartExplorerPage.page.getByTestId('chart-type-table'),
      ).toBeVisible();
      await expect(
        chartExplorerPage.page.getByTestId('chart-type-number'),
      ).toBeVisible();
    });
  });
