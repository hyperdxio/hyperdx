 ...
 * separately, carries its own alias (prefixed onto its series names) and its
 * own legend template, and is listed in the Generated PromQL preview.
 *
  ...
  () => {
    test('plots, names, previews and round-trips two expressions', async ({
      ...
      await test.step('Create a dashboard with a two-expression PromQL tile', async () => {
        ...
        await editor.promqlAliasInput(0).fill('up-acct');
        await editor.promqlAliasInput(1).fill('up-api');
      });
      ...
      await test.step('Each expression names its series with its alias', async () => {
        await editor.save();
        await expect(dashboardPage.getTiles()).toHaveCount(1, {
          timeout: 10000,
        });
        // Each expression returns one series, so its labels are all shared and
        // the default name is the bare metric name behind the alias.
        for (const alias of ['up-acct', 'up-api']) {
          await expect(
            legend.getByText(`${alias} · ${E2E_PROMQL_METRIC_NAME}`),
          ).toBeVisible({ timeout: 30000 });
        }
      });

      await test.step("An expression's own legend template wins over the chart's", async () => {
        await dashboardPage.editTile(0);
        await expect(editor.nameInput).toBeVisible();
        await editor.setLegendTemplate('chart:{{service}}');
        await editor.promqlLegendTemplateInput(0).fill('own:{{service}}');
        await editor.save();
        await expect(dashboardPage.getTiles()).toHaveCount(1, {
          timeout: 10000,
        });
        await expect(legend.getByText('up-acct · own:accounting')).toBeVisible({
          timeout: 30000,
        });
        await expect(
      ...
      await test.step('Both expressions round-trip into the editor', async () => {
        ...
        await expect(editor.promqlAliasInput(0)).toHaveValue('up-acct');
        await expect(editor.promqlAliasInput(1)).toHaveValue('up-api');
        await expect(editor.promqlLegendTemplateInput(0)).toHaveValue(
          'own:{{service}}',
        );
        await expect(editor.promqlLegendTemplateInput(1)).toHaveValue('');
      });

      await test.step('Removing an expression leaves the other plotted', async () => {
        await page
          .getByTestId('promql-remove-expression-button')
          .first()
          .click();
        await editor.save();
        await expect(dashboardPage.getTiles()).toHaveCount(1, {
