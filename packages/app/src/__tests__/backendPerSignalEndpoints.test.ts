describe('@hyperdx/node-opentelemetry per-signal endpoint support', () => {
  ...
  it('should use OTEL_EXPORTER_OTLP_TRACES_ENDPOINT over base endpoint', () => {
    process.env.OTEL_EXPORTER_OTLP_ENDPOINT = 'http://base-collector:4318';
    process.env.OTEL_EXPORTER_OTLP_TRACES_ENDPOINT =
      'http://traces-collector:4318/v1/traces';

    const constants = require('@hyperdx/node-opentelemetry/build/src/constants');
    expect(constants.DEFAULT_OTEL_TRACES_EXPORTER_URL).toBe(
      'http://traces-collector:4318/v1/traces',
    );
  });

  it('should use OTEL_EXPORTER_OTLP_LOGS_ENDPOINT over base endpoint', () => {
    process.env.OTEL_EXPORTER_OTLP_ENDPOINT = 'http://base-collector:4318';
  ...
  it('should use OTEL_EXPORTER_OTLP_METRICS_ENDPOINT over base endpoint', () => {
    process.env.OTEL_EXPORTER_OTLP_ENDPOINT = 'http://base-collector:4318';
  ...
  it('should derive signal URLs from base endpoint when per-signal vars are not set', () => {
    process.env.OTEL_EXPORTER_OTLP_ENDPOINT = 'http://base-collector:4318';
  ...
  it('should allow mixing per-signal and base endpoints', () => {
    process.env.OTEL_EXPORTER_OTLP_ENDPOINT = 'http://base-collector:4318';
    process.env.OTEL_EXPORTER_OTLP_TRACES_ENDPOINT =
  ...
  it('should use base endpoint as-is for gRPC protocol (no path suffix)', () => {
