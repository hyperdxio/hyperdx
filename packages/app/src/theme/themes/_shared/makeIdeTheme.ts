...
export interface IdeThemeOptions {
  primaryColor: string;
...
/**
 * Shared Mantine theme factory for IDE-inspired themes.
 *
 * Body Mantine component overrides are byte-identical across nord, catppuccin
 * and onedark; only `primaryColor`, `primaryShade` and the `colors` palette
 * differ. Centralising the overrides here keeps the per-theme files small and
 * means a fix only has to be applied once.
 */
export function makeIdeTheme({
  primaryColor,
  primaryShade,
  colors,
}: IdeThemeOptions): MantineThemeOverride {
  return {
    cursorType: 'pointer',
    defaultRadius: 'sm',
    ...
    components: {
      Tooltip: Tooltip.extend({
        styles: () => ({
          tooltip: { fontFamily: 'var(--mantine-font-family)' },
        }),
      }),
      Modal: {
        styles: {
      ...
      InputWrapper: {
        styles: {
          ...
          description: { marginBottom: 8, lineHeight: 1.3 },
        },
      },
      Select: Select.extend({
        styles: { input: { border: '1px solid var(--color-border)' } },
      }),
      Slider: Slider.extend({
        vars: () => ({ root: { '--slider-color': 'var(--color-slider-bar)' } }),
        styles: {
          thumb: {
        ...
        classNames: { mark: componentClasses.sliderMark },
      }),
      Input: {
        styles: {
          input: {
      ...
      Card: {
        styles: (_theme: MantineTheme, props: { variant?: string }) => {
          ...
          return { root: { backgroundColor: 'var(--color-bg-body)' } };
        },
      },
      Divider: {
        styles: {
      ...
      Accordion: {
        styles: (_theme: MantineTheme, props: { variant?: string }) => {
          const base = {
            control: { '--item-border-color': 'var(--color-border)' },
            item: { borderColor: 'var(--color-border)' },
          };
      ...
      UnstyledButton: {
        styles: { root: { '--item-border-color': 'var(--color-border)' } },
      },
      Paper: {
        classNames: (_theme: MantineTheme, props: { variant?: string }) => {
          if (props.variant === 'muted') return { root: 'paper-muted' };
          return {};
        },
        styles: (_theme: MantineTheme, props: { variant?: string }) => {
          if (props.variant === 'muted') {
      ...
      Tabs: Tabs.extend({
        vars: () => ({ root: { '--tabs-color': 'var(--color-text-brand)' } }),
