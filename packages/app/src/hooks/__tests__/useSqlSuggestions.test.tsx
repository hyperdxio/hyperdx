...
describe('useSqlSuggestions', () => {
  ...
  it('should handle multiple double quotes in input', () => {
    const { result } = renderHook(() =>
      useSqlSuggestions({
        input:
          'SeverityText = "error" OR SeverityText = "info" OR SeverityText = "debug" OR SeverityText = "warn"',
        enabled: true,
      }),
    );
    ...
    const suggestion = suggestions[0];
    expect(suggestion.corrected()).toBe(
      "SeverityText = 'error' OR SeverityText = 'info' OR SeverityText = 'debug' OR SeverityText = 'warn'",
    );
  });

  it('should update suggestions when input changes', () => {
    const { result, rerender } = renderHook(props => useSqlSuggestions(props), {
  ...
  it('should handle input with escaped single quotes', () => {
    ...
    expect(result.current).not.toBeNull();
    expect(result.current?.[0].corrected()).toBe(
      "SeverityText = 'O\\'Reilly' AND company = 'Acme'",
    );
  });
  ...
  it('should handle mixed quotes correctly', () => {
    ...
    expect(result.current).not.toBeNull();
    expect(result.current?.[0].corrected()).toBe(
      "SeverityText = 'single' OR SeverityText = 'double'",
    );
  });
});
