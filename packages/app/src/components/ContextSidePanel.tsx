...
export default function ContextSubpanel({
  ...
  const handleRowExpandClick = useCallback(
    (rowWhere: RowWhereResult, row: Record<string, any>) => {
      ...
      const fallback = isTraceSource(source)
        ? t('context.span')
        : t('context.log');
      const label =
  ...
  */
  ...
  function generateSegmentedControlData() {
    return [
      { label: t('context.all'), value: ContextBy.All },
      ...(service
        ? [{ label: t('context.service'), value: ContextBy.Service }]
        : []),
      ...(host ? [{ label: t('context.host'), value: ContextBy.Host }] : []),
      ...(k8sPodName
        ? [{ label: t('context.pod'), value: ContextBy.Pod }]
        : []),
      ...(k8sNodeName
        ? [{ label: t('context.node'), value: ContextBy.Node }]
        : []),
      { label: t('context.custom'), value: ContextBy.Custom },
    ];
  }
        ...
        >
          <Group justify="space-between" p="sm">
            ...
            <SegmentedControl
              ...
              data={[
                {
                  label: t('context.duration100ms'),
                  value: ms('100ms').toString(),
                },
                {
                  label: t('context.duration500ms'),
                  value: ms('500ms').toString(),
                },
                { label: t('context.duration1s'), value: ms('1s').toString() },
                { label: t('context.duration5s'), value: ms('5s').toString() },
                {
                  label: t('context.duration30s'),
                  value: ms('30s').toString(),
                },
                { label: t('context.duration1m'), value: ms('1m').toString() },
                { label: t('context.duration5m'), value: ms('5m').toString() },
                {
                  label: t('context.duration15m'),
          ...
          <Group p="sm">
            <div>
              ...
              <Badge size="md" variant="default">
                {t('context.timeRange', { range: ms(range / 2) })}
              </Badge>
            </div>
          </Group>
