...
describe('computeComparisonScore', () => {
  ...
  it('returns 0 when both groups have identical proportions', () => {
    const outlier = new Map([['GET', 80], ['POST', 20]]);
    const inlier = new Map([['GET', 40], ['POST', 10]]);
    expect(computeComparisonScore(outlier, inlier)).toBeCloseTo(0);
  });

  it('returns high score for different proportions', () => {
    const outlier = new Map([['error', 90], ['ok', 10]]);
    const inlier = new Map([['error', 10], ['ok', 90]]);
  ...
  it('normalizes by group sum so different sample sizes produce same score', () => {
    const outlierSmall = new Map([['GET', 8], ['POST', 2]]);
    const outlierLarge = new Map([['GET', 800], ['POST', 200]]);
    const inlier = new Map([['GET', 50], ['POST', 50]]);
...
describe('computeEntropyScore', () => {
  ...
  it('returns ~0 for perfectly uniform distribution', () => {
    expect(computeEntropyScore(new Map([['a', 50], ['b', 50]]))).toBeCloseTo(0);
  });
  ...
  it('returns ~0 for uniform 3-value field', () => {
    expect(computeEntropyScore(new Map([['a', 33.33], ['b', 33.33], ['c', 33.34]]))).toBeCloseTo(0, 2);
  });

  it('handles power-law distributions', () => {
    const powerLaw = new Map([['v1', 50], ['v2', 25], ['v3', 12], ['v4', 6], ['v5', 4], ['v6', 2], ['v7', 1]]);
