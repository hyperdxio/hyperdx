...
overridden to any palette token, and you can add a reference line (with an
optional label and color) to mark a value such as a 0 error budget, an SLA, or
a target. Configure it under Display Settings > Background chart on a number
tile. Available on builder number tiles (raw SQL number tiles return a single
value with no time dimension to bucket).
