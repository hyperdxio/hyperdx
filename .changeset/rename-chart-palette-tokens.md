...
Stored configs from the initial color picker (#2265) keep working — a Zod `preprocess` on `ChartPaletteTokenSchema` transparently maps legacy `chart-1` .. `chart-10` to their hue-named equivalents at parse time, preserving the HyperDX slot ordering (slot 1 = brand green, slot 2 = blue, etc.).
