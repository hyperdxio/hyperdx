...
[`packages/app/src/components/GettingStarted/IntegrationsDrawer.tsx`](../packages/app/src/components/GettingStarted/IntegrationsDrawer.tsx)
...
that content is generated at build time from the SDK MDX in
...
- **HyperDX onboarding** (current consumer) — the integrations drawer.
...
- **Today:** heuristic block classification + a placeholder regex list +
  per-page quirks. Works, but fragile and needs babysitting when docs change.
...
- [ ] Align SDK ids/categories with the app catalog in
      [`integrationsCatalog.tsx`](../packages/app/src/components/GettingStarted/integrationsCatalog.tsx).
